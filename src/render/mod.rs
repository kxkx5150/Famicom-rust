...
pub fn render_tile(
    ...
    shift_y: isize,
    tile_row: usize,
) ->(isize, isize, (u8, u8, u8)){
    let bank = ppu.ctrl.bknd_pattern_addr();
    let attribute_table = &name_table[0x3c0..0x400];

    for tile_column in 0..32 {
        let tileno = tile_column+32*tile_row;

        // println!("\ntile_column  {}",tile_column);
        // println!("tile_row     {}",tile_row);
        // println!("tile_no      {}",tileno);

        let tile_idx = name_table[(tile_column + tileno) as usize] as u16;
        let tile =
            &ppu.chr_rom[(bank + tile_idx * 16) as usize..=(bank + tile_idx * 16 + 15) as usize];
        let palette = bg_pallette(ppu, attribute_table, tile_column as usize, tile_row);
        ...
        for y in 0..=7 {
            ...
            for x in (0..=7).rev() {
                ...
                {
                    return  (
                        (shift_x + pixel_x as isize),
                        (shift_y + pixel_y as isize),
                        rgb,
                    );
                }
            }
